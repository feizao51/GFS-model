...
class TruncatedBoomerang(Wordwarp):
    ...
    def generate_upper_random_constraints(self):
        ...
        for rn in range (1, self.rm):
            ...
            for i in range (32):
                y = self.ipermute_nibbles[i]
                if i & 1 == 0:
                    constraints += f"{xu[y]} + {uran0[y - 1]} - {uran[i]} >= 0\n"
                    constraints += f"{uran0[y]} + {xu[y - 1]} - {uran[i]} >= 0\n"
                    constraints += f"{uran[i]} - {xu[y]} - {xu[y - 1]} >= -1\n"
                    constraints += f"{uran[i]} - {uran0[y]} >= 0\n"
                    constraints += f"{uran[i]} - {uran0[y - 1]} >= 0\n"
                else:
                    constraints += f"{uran[i]} - {uran0[y]} = 0\n"
        return constraints
    ...
    def generate_lower_random_constraints(self):
        ...
        for r in range (1, self.rm):
            ...
            xl = self.generate_round_x_variables(rn + 1, ul='l')
            for i in range (32):
                y = self.permute_nibbles[i]
                if i & 1 == 1:
                    y0 = self.permute_nibbles[i - 1]
                    constraints += f"{xl[y]} + {lran0[y0]} - {lran[i]} >= 0\n"
                    constraints += f"{lran0[y]} + {xl[y0]} - {lran[i]} >= 0\n"
                    constraints += f"{lran[i]} - {xl[y]} - {xl[y0]} >= -1\n"
                    constraints += f"{lran[i]} - {lran0[y]} >= 0\n"
                    constraints += f"{lran[i]} - {lran0[y0]} >= 0\n"
                else:
                    constraints += f"{lran[i]} - {lran0[y]} = 0\n"
        return constraints
    ...
    def generate_upper_used_constraints(self):
        ...
        for rn in range (self.rm - 1):
            for i in range (16):
                temp = ""
                y = self.permute_nibbles[(i << 1) + 1]
                constraints += f"- {uisu[rn][i]} - {uran[rn][i << 1]} >= -1\n"
                constraints += f"xu_{self.r0 + rn}_{i << 1} - {uisu[rn][i]} >= 0\n"
                for rs in range (rn + 1, self.rm, 2):
                    constraints += f"{uisu[rn][i]} - {svar[rs][y >> 1]} + {uran[rs][y]} - xu_{self.r0 + rn}_{i << 1} >= -1\n"
                    constraints += f"{uisu[rn][i]} - {uisu[rs][y >> 1]} + {uran[rs][y]} - xu_{self.r0 + rn}_{i << 1} >= -1\n"
                    constraints += f"- {uisu[rn][i]} - {uran[rs][y]}" + temp + " >= -1\n"
                    temp += f" + {svar[rs][y >> 1]} + {uisu[rs][y >> 1]}"
                    y = self.permute_nibbles[self.permute_nibbles[y]]
    ...
    def generate_lower_used_constraints(self):
        ...
        for rn in range (1, self.rm):
            for i in range (16):
                temp = ""
                y = (i << 1) + 1
                constraints += f"- {lisu[rn][i]} - {lran[rn][i << 1]} >= -1\n"
                constraints += f"xl_{rn}_{i << 1} - {lisu[rn][i]} >= 0\n"
                for r in range (0, rn, 2):
                    rs = rn - r
                    y0 = self.ipermute_nibbles[y]
                    constraints += f"{lisu[rn][i]} - {svar[rs - 1][y0 >> 1]} + {lran[rs][y]} - xl_{rn}_{i << 1} >= -1\n"
                    constraints += f"{lisu[rn][i]} - {lisu[rs - 1][y0 >> 1]} + {lran[rs][y]} - xl_{rn}_{i << 1} >= -1\n"
                    constraints += f"- {lisu[rn][i]} - {lran[rs][y]}" + temp + " >= -1\n"
                    temp += f" + {svar[rs - 1][y0 >> 1]} + {lisu[rs - 1][y0 >> 1]}"
                    y = self.ipermute_nibbles[y0]
    ...
    def generate_first_round_constraints(self):
        xu = self.generate_round_x_variables(0, ul='u')
        constraints = f"xu_{0}_0"
        for i in range (1, 32):
            constraints += f" + {xu[i]}"
        constraints += " <= 7\n"
        return constraints
    
    def generate_last_round_constraints(self):
        xl = self.generate_round_x_variables(self.R1, ul='l')
        constraints = f"xl_{self.R1}_0"
        for i in range (1, 32):
            constraints += f" + {xl[i]}"
        constraints += " <= 9\n"
        return constraints
    ...
    def make_model(self):
        ...
        #constraints += self.generate_first_round_constraints()
        #constraints += self.generate_last_round_constraints()
    ...
    def parse_solver_output(self):
        ...
        cond = []
        for r in range(self.rm):
            cond.extend(self.generate_tables_vars(r, "DDT"))
        nus = sum((list(map(get_value_int, cond))))
        self.middle_part["us"] = nus
